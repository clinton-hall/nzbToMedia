...
nzbToMedia() {
	...
	if [ "$NZBPP_CATEGORY" = "$CouchPotatoCategory" ]; then
		if [ "$CouchPotato" = "yes" -a -e "$NzbToCouchPotato" ]; then
			...
			$PythonCmd $NzbToCouchPotato "$NZBPP_DIRECTORY" "$NZBPP_NZBFILENAME" "$NZBPP_CATEGORY" "$PostProcessStatus" | while read line ; do if [ "$line" != "" ] ; then replaceLogLine "${line}" ; fi ; done
		else
	...
	if [ "$NZBPP_CATEGORY" = "$SickBeardCategory" ]; then
		if [ "$SickBeard" = "yes" -a -e "$NzbToSickBeard" ]; then
			...
			$PythonCmd $NzbToSickBeard "$NZBPP_DIRECTORY" "$NZBPP_NZBFILENAME" "$NZBPP_CATEGORY" "$PostProcessStatus" | while read line ; do if [ "$line" != "" ] ; then replaceLogLine "${line}" ; fi ; done
		else
	...
	if [ "$NZBPP_CATEGORY" = "$HeadPhonesCategory" ]; then
		if [ "$HeadPhones" = "yes" -a -e "$NzbToHeadPhones" ]; then
			...
			$PythonCmd $NzbToHeadPhones "$NZBPP_DIRECTORY" "$NZBPP_NZBFILENAME" "$NZBPP_CATEGORY" "$PostProcessStatus" | while read line ; do if [ "$line" != "" ] ; then replaceLogLine "${line}" ; fi ; done
		else
	...
	if [ "$NZBPP_CATEGORY" = "$MylarCategory" ]; then
		if [ "$Mylar" = "yes" -a -e "$NzbToMylar" ]; then
			...
			$PythonCmd $NzbToMylar "$NZBPP_DIRECTORY" "$NZBPP_NZBFILENAME" "$NZBPP_CATEGORY" "$PostProcessStatus" | while read line ; do if [ "$line" != "" ] ; then replaceLogLine "${line}" ; fi ; done
		else
	...
	if [ "$NZBPP_CATEGORY" = "$GamezCategory" ]; then
		if [ "$Gamez" = "yes" -a -e "$NzbToGamez" ]; then
			...
			$PythonCmd $NzbToGamez "$NZBPP_DIRECTORY" "$NZBPP_NZBFILENAME" "$NZBPP_CATEGORY" "$PostProcessStatus" | while read line ; do if [ "$line" != "" ] ; then replaceLogLine "${line}" ; fi ; done
		else
	...
	if [ "$NZBPP_CATEGORY" = "$CustomCategory" ]; then
		if [ "$Custom" = "yes" -a -e "$CustomScript" ]; then
			...
			$CustomCmd $CustomScript "$NZBPP_DIRECTORY" "$NZBPP_NZBFILENAME" "$NZBPP_CATEGORY" "$PostProcessStatus" | while read line ; do if [ "$line" != "" ] ; then replaceLogLine "${line}" ; fi ; done
		else
